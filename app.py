...
def debounce_render(key: str, delay: float = 0.15) -> None:
    """Ensure renders for ``key`` are throttled by ``delay`` seconds."""

    state_key = f"_{key}_last_render"
    now = perf_counter()
    last = st.session_state.get(state_key, 0.0)
    elapsed = now - last
    if elapsed < delay:
        sleep(delay - elapsed)
    st.session_state[state_key] = perf_counter()
...
.chart-toolbar {
  ...
  backdrop-filter: blur(6px);
  box-shadow: 0 10px 26px rgba(var(--primary-rgb,11,31,59),0.08);
}
.chart-toolbar .stColumns {
  gap: 0.75rem;
  width: 100%;
}
.chart-toolbar .stColumn {
  flex: 1 1 260px;
  min-width: 240px;
  max-width: 320px;
}
.chart-toolbar .toolbar-control {
  display: flex;
  flex-direction: column;
  gap: 0.35rem;
}
...
.chart-toolbar .value-badge {
  border-radius: 999px;
  padding: 0.1rem 0.55rem;
  background: rgba(var(--accent-rgb,30,136,229),0.16);
  color: var(--primary,#0B1F3B);
  font-weight: 600;
  font-variant-numeric: tabular-nums;
}
.chart-toolbar .stSlider,
.chart-toolbar .stRadio,
.chart-toolbar .stNumberInput,
.chart-toolbar .stSelectbox,
.chart-toolbar .stTextInput {
  margin-bottom: 0 !important;
}
.chart-accordion {
  margin-top: 0.35rem;
}
.chart-accordion .stExpander {
  border: 1px solid rgba(var(--primary-rgb,11,31,59),0.14);
  border-radius: 12px;
  background: rgba(255,255,255,0.9);
  box-shadow: 0 10px 24px rgba(var(--primary-rgb,11,31,59),0.08);
}
.chart-accordion .stExpander .streamlit-expanderHeader {
  font-weight: 600;
  color: var(--ink,var(--primary,#0B1F3B));
}
@media (max-width: 1023px) {
  .chart-toolbar { top: 0.5rem; }
}
@media (max-width: 767px) {
  ...
  .chart-toolbar::after {
    ...
    st.markdown('<section class="chart-card" aria-label="メインチャート設定カード">', unsafe_allow_html=True)

    st.markdown('<div class="chart-toolbar" role="group" aria-label="主要設定">', unsafe_allow_html=True)
    ...
    with toolbar_cols[0]:
        ...
        header.markdown(
            f'<div class="toolbar-label">形状抽出の感度<span class="value-badge">{_format_float(sens)}</span></div>',
            unsafe_allow_html=True,
        )
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with toolbar_cols[1]:
        ...
        header.markdown(
            f'<div class="toolbar-label">急勾配 zスコア<span class="value-badge">{_format_float(z_thr, 2)}</span></div>',
            unsafe_allow_html=True,
        )
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with toolbar_cols[2]:
        ...
        header.markdown(
            f'<div class="toolbar-label">形状抽出<span class="value-badge">{html.escape(shape_pick.replace("（", "").replace("）", ""))}</span></div>',
            unsafe_allow_html=True,
        )
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with toolbar_cols[3]:
        ...
        if amount_slider_cfg:
            ...
            badge = "設定なし"
        header.markdown(
            f'<div class="toolbar-label">金額レンジ<span class="value-badge">{badge}</span></div>',
            unsafe_allow_html=True,
        )
        if band_mode != "金額指定":
    ...
    with chart_body_placeholder:
        st.markdown(
            '<div class="chart-body" role="region" aria-live="polite">',
            unsafe_allow_html=True,
        )
